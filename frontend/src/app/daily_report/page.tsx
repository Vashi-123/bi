...
import { ArrowLeft, Plus, Trash2, Send, ShieldCheck, UserCircle, Edit3, XCircle, Package } from 'lucide-react';
import Link from 'next/link';

type SettingCategory = 'report_recipients' | 'authorized_users';
...
export default function DailyReportPage() {
    ...
    const { data: settingsData, isLoading: settingsLoading } = useSWR(
        `${API_BASE}/api/settings/${activeCategory === 'report_recipients' ? 'report_recipients' : 'admins'}`, 
        fetcher
    );
    ...
    const handleSaveSingle = async () => {
        ...
        setIsSaving(true);
        try {
            const endpoint = activeCategory === 'report_recipients' ? 'report_recipients' : 'admins';
            const payload = { 
            ...
            <main className="relative z-10 max-w-6xl mx-auto space-y-10">
                ...
                <Flex justifyContent="between" className="items-center">
                    <div className="space-y-1">
                        ...
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.3em]">Manage Distribution and Analytics Access</p>
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-3">
                        <Link href="/stock_settings" className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 hover:border-[#0C0C0C] rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm group">
                            ...
                            Switch to Stock Settings
                        </Link>
                        {/* Tab Switcher */}
                        <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 flex gap-1">
                        <button 
                            onClick={() => { setActiveCategory('report_recipients'); cancelEditing(); }}
                            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition-all
                                ${activeCategory === 'report_recipients' ? 'bg-[#0C0C0C] text-white shadow-lg' : 'text-slate-400 hover:text-[#0C0C0C]'}`}
                        >
                            <Send className="w-4 h-4" /> Recipients
                        </button>
                        <button 
                            onClick={() => { setActiveCategory('authorized_users'); cancelEditing(); }}
                            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition-all
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <Card className="lg:col-span-1 rounded-3xl border-slate-100 shadow-xl p-8 bg-white h-fit relative">
                        ...
                        <Title className="text-xl font-bold mb-6 text-[#0C0C0C]">
                            {editingId ? 'Edit Entry' : `Add ${activeCategory === 'report_recipients' ? 'Recipient' : 'Admin'}`}
                        </Title>
                        <div className="space-y-6">
                            ...
                            >
                                ...
                                {editingId ? 'Update Entry' : `Add to ${activeCategory === 'report_recipients' ? 'Distribution' : 'Admins'}`}
                            </button>
                        </div>
                    </Card>
                    ...
                    <Card className="lg:col-span-2 rounded-3xl border-slate-100 shadow-xl p-8 bg-white overflow-hidden">
                        <Title className="text-xl font-bold mb-8 text-[#0C0C0C]">
                            {activeCategory === 'report_recipients' ? 'Notification List' : 'Authorized Admins'}
                        </Title>
                        <div className="max-h-[500px] overflow-y-auto pr-2">
                            <Table>
                                <TableHead className="bg-white sticky top-0 z-10 shadow-sm">
                                    <TableRow className="border-b border-slate-100">
                                        <TableHeaderCell className="text-[10px] font-bold !text-slate-500 uppercase tracking-widest py-4">Avatar</TableHeaderCell>
                                ...
                                <TableBody>
                                    {settingsLoading ? (
                                        <TableRow>
                                            <TableCell colSpan={3} className="py-20 text-center text-slate-300 font-bold text-xs animate-pulse">Loading settings...</TableCell>
                                        </TableRow>
